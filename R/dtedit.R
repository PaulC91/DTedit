...
dtedit <- function(input,
  ...
  if (!missing(input.types)) {
    if (!all(names(input.types) %in% edit.cols)) {
      stop(
        "input.types column not a valid editting column: ",
  ...
  output[[name]] <- shiny::renderUI({
    shiny::div(
      if (show.insert) {
        shiny::actionButton(paste0(id, name, "_add"), label.add)
      },
      if (show.update) {
        shiny::actionButton(paste0(id, name, "_edit"), label.edit)
      },
      if (show.delete) {
        shiny::actionButton(paste0(id, name, "_remove"), label.delete)
      },
      if (show.copy) {
